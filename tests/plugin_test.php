...
/**
 * metagroup enrolment sync functional test.
 *
...
class enrol_metagroup_plugin_testcase extends advanced_testcase {

    protected function enable_plugin() {
        $enabled = enrol_get_plugins(true);
        $enabled['metagroup'] = true;
    ...
    protected function disable_plugin() {
        $enabled = enrol_get_plugins(true);
        unset($enabled['metagroup']);
        $enabled = array_keys($enabled);
        set_config('enrol_plugins_enabled', implode(',', $enabled));
    }

    protected function is_metagroup_enrolled($user, $enrol, $role = null) {
        global $DB;
    ...
    protected function has_role($user, $enrol, $role) {
        ...
        if ($role === false) {
            if ($DB->record_exists('role_assignments', array('contextid'=>$context->id, 'userid'=>$user->id, 'component'=>'enrol_metagroup', 'itemid'=>$enrol->id))) {
                return false;
            }
        } else if (!$DB->record_exists('role_assignments', array('contextid'=>$context->id, 'userid'=>$user->id, 'roleid'=>$role->id, 'component'=>'enrol_metagroup', 'itemid'=>$enrol->id))) {
            return false;
        }
    ...
    public function test_sync() {
        ...
        $metagrouplplugin = enrol_get_plugin('metagroup');
        ...
        set_config('syncall', 0, 'enrol_metagroup');
        set_config('nosyncroleids', $manager->id, 'enrol_metagroup');

        require_once($CFG->dirroot.'/enrol/metagroup/locallib.php');

        enrol_metagroup_sync(null, false);
        $this->assertEquals(7, $DB->count_records('user_enrolments'));
        $this->assertEquals(6, $DB->count_records('role_assignments'));
        ...
        // Disable the plugin to prevent add_instance from calling enrol_metagroup_sync.
        $this->disable_plugin();
        $e1 = $metagrouplplugin->add_instance($course3, array('customint1'=>$course1->id));
        $e2 = $metagrouplplugin->add_instance($course3, array('customint1'=>$course2->id));
        $e3 = $metagrouplplugin->add_instance($course4, array('customint1'=>$course2->id));
        $enrol1 = $DB->get_record('enrol', array('id'=>$e1));
        $enrol2 = $DB->get_record('enrol', array('id'=>$e2));
        $enrol3 = $DB->get_record('enrol', array('id'=>$e3));
        $this->enable_plugin();

        enrol_metagroup_sync($course4->id, false);
        $this->assertEquals(9, $DB->count_records('user_enrolments'));
        $this->assertEquals(8, $DB->count_records('role_assignments'));
        $this->assertTrue($this->is_metagroup_enrolled($user1, $enrol3, $student));
        $this->assertTrue($this->is_metagroup_enrolled($user2, $enrol3, $teacher));

        enrol_metagroup_sync(null, false);
        $this->assertEquals(14, $DB->count_records('user_enrolments'));
        $this->assertEquals(13, $DB->count_records('role_assignments'));

        $this->assertTrue($this->is_metagroup_enrolled($user1, $enrol1, $student));
        $this->assertTrue($this->is_metagroup_enrolled($user2, $enrol1, $student));
        $this->assertFalse($this->is_metagroup_enrolled($user3, $enrol1));
        $this->assertTrue($this->is_metagroup_enrolled($user4, $enrol1, $teacher));
        $this->assertFalse($this->is_metagroup_enrolled($user5, $enrol1));

        $this->assertTrue($this->is_metagroup_enrolled($user1, $enrol2, $student));
        $this->assertTrue($this->is_metagroup_enrolled($user2, $enrol2, $teacher));

        $this->assertTrue($this->is_metagroup_enrolled($user1, $enrol3, $student));
        $this->assertTrue($this->is_metagroup_enrolled($user2, $enrol3, $teacher));

        set_config('syncall', 1, 'enrol_metagroup');
        enrol_metagroup_sync(null, false);
        $this->assertEquals(16, $DB->count_records('user_enrolments'));
        $this->assertEquals(13, $DB->count_records('role_assignments'));

        $this->assertTrue($this->is_metagroup_enrolled($user3, $enrol1, false));
        $this->assertTrue($this->is_metagroup_enrolled($user5, $enrol1, false));
        ...
        set_config('unenrolaction', ENROL_EXT_REMOVED_SUSPEND, 'enrol_metagroup');
        enrol_metagroup_sync($course4->id, false);
        $this->assertEquals(14, $DB->count_records('user_enrolments'));
        $this->assertEquals(11, $DB->count_records('role_assignments'));
        ...
        set_config('unenrolaction', ENROL_EXT_REMOVED_SUSPENDNOROLES, 'enrol_metagroup');
        enrol_metagroup_sync($course4->id, false);
        $this->assertEquals(14, $DB->count_records('user_enrolments'));
        $this->assertEquals(10, $DB->count_records('role_assignments'));
        $this->assertEquals(11, $DB->count_records('user_enrolments', array('status'=>ENROL_USER_ACTIVE)));
        $this->assertTrue($this->is_metagroup_enrolled($user1, $enrol3, false));
        $this->assertTrue($DB->record_exists('user_enrolments', array('enrolid'=>$enrol3->id, 'status'=>ENROL_USER_SUSPENDED, 'userid'=>$user1->id)));
        ...
        $this->assertEquals(8, $DB->count_records('role_assignments'));
        $this->assertEquals(11, $DB->count_records('user_enrolments', array('status'=>ENROL_USER_ACTIVE)));
        $this->assertTrue($this->is_metagroup_enrolled($user1, $enrol1, false));
        $this->assertTrue($DB->record_exists('user_enrolments', array('enrolid'=>$enrol1->id, 'status'=>ENROL_USER_SUSPENDED, 'userid'=>$user1->id)));
        $this->assertTrue($this->is_metagroup_enrolled($user1, $enrol2, false));
        $this->assertTrue($DB->record_exists('user_enrolments', array('enrolid'=>$enrol2->id, 'status'=>ENROL_USER_SUSPENDED, 'userid'=>$user1->id)));

        set_config('unenrolaction', ENROL_EXT_REMOVED_UNENROL, 'enrol_metagroup');
        enrol_metagroup_sync($course4->id, false);
        $this->assertEquals(13, $DB->count_records('user_enrolments'));
        $this->assertEquals(8, $DB->count_records('role_assignments'));
        $this->assertEquals(11, $DB->count_records('user_enrolments', array('status'=>ENROL_USER_ACTIVE)));
        $this->assertFalse($this->is_metagroup_enrolled($user1, $enrol3));

        enrol_metagroup_sync(null, false);
        $this->assertEquals(11, $DB->count_records('user_enrolments'));
        $this->assertEquals(8, $DB->count_records('role_assignments'));
        $this->assertEquals(11, $DB->count_records('user_enrolments', array('status'=>ENROL_USER_ACTIVE)));
        $this->assertFalse($this->is_metagroup_enrolled($user1, $enrol1));
        $this->assertFalse($this->is_metagroup_enrolled($user1, $enrol2));
        ...
        $this->assertFalse($this->is_metagroup_enrolled($user1, $enrol1, $student));
        enrol_metagroup_sync(null, false);
        $this->assertEquals(10, $DB->count_records('user_enrolments'));
        $this->assertEquals(8, $DB->count_records('role_assignments'));
        $this->assertEquals(10, $DB->count_records('user_enrolments', array('status'=>ENROL_USER_ACTIVE)));
        ...
        $this->assertEquals(10, $DB->count_records('role_assignments'));
        $this->assertEquals(11, $DB->count_records('user_enrolments', array('status'=>ENROL_USER_ACTIVE)));
        $this->assertTrue($this->is_metagroup_enrolled($user1, $enrol1, $teacher));
        enrol_metagroup_sync(null, false);
        $this->assertEquals(11, $DB->count_records('user_enrolments'));
        $this->assertEquals(10, $DB->count_records('role_assignments'));
        $this->assertEquals(11, $DB->count_records('user_enrolments', array('status'=>ENROL_USER_ACTIVE)));
    ...
    public function test_add_to_group() {
        ...
        $e1 = $metagrouplplugin->add_instance($course3, array('customint1' => $course1->id, 'customint2' => $group31->id));
        $e2 = $metagrouplplugin->add_instance($course3, array('customint1' => $course2->id, 'customint2' => $group32->id));
        ...
        $this->assertTrue($DB->record_exists('groups_members', array('groupid' => $group31->id, 'userid' => $user1->id,
            'component' => 'enrol_metagroup', 'itemid' => $e1)));
        ...
        $this->assertTrue($DB->record_exists('groups_members', array('groupid' => $group32->id, 'userid' => $user1->id,
            'component' => 'enrol_metagroup', 'itemid' => $e2)));
    ...
    public function test_add_to_group_with_member() {
        ...
        // Add instance of metagroup enrolment in course2 linking to course1 and enrol both users in course1.
        $metagrouplplugin->add_instance($course2, array('customint1' => $course1->id, 'customint2' => $groupid));
    ...
    public function test_user_enrolment_created_event() {
        ...
        $metagroupplugin = enrol_get_plugin('metagroup');
        $user1 = $this->getDataGenerator()->create_user();
        $course1 = $this->getDataGenerator()->create_course();
        $course2 = $this->getDataGenerator()->create_course();
        $student = $DB->get_record('role', array('shortname' => 'student'));

        $e1 = $metagroupplugin->add_instance($course2, array('customint1' => $course1->id));
        $enrol1 = $DB->get_record('enrol', array('id' => $e1));

        // Enrol user and capture event.
        $sink = $this->redirectEvents();

        $metagroupplugin->enrol_user($enrol1, $user1->id, $student->id);
        ...
        $expectedlegacyeventdata->enrol = 'metagroup';
    ...
    public function test_user_enrolment_deleted_event() {
        ...
        $e1 = $metagrouplplugin->add_instance($course2, array('customint1' => $course1->id));
        $enrol1 = $DB->get_record('enrol', array('id' => $e1));

        // Enrol user.
        $metagrouplplugin->enrol_user($enrol1, $user1->id, $student->id);
        $this->assertEquals(1, $DB->count_records('user_enrolments'));

        // Unenrol user and capture event.
        $sink = $this->redirectEvents();
        $metagrouplplugin->unenrol_user($enrol1, $user1->id);
        $events = $sink->get_events();
        $sink->close();
    ...
    public function test_user_enrolment_updated_event() {
        ...
        $sink = $this->redirectEvents();
        $metagrouplplugin->update_user_enrol($enrol1, $user1->id, ENROL_USER_SUSPENDED, null, time());
        $events = $sink->get_events();
        $sink->close();
        $event = array_shift($events);
     ...
     */
    public function test_enrol_metagroup_create_new_group() {
        global $DB;
        $this->resetAfterTest();
        // Create two courses.
        ...
        $metagroupcourse = $this->getDataGenerator()->create_course(array('fullname' => 'All sciences'));
        // Run the function.
        $groupid = enrol_metagroup_create_new_group($metagroupcourse->id, $course->id);
        // Check the results.
        ...
        $this->assertEquals($metagroupcourse->id, $group->courseid);

        // Create a group that will have the same name as the course.
        $groupdata = new stdClass();
        $groupdata->courseid = $metagroupcourse->id;
        $groupdata->name = 'Physics course';
        groups_create_group($groupdata);
        // Create a group for the course 2 in metagroupcourse.
        $groupid = enrol_metagroup_create_new_group($metagroupcourse->id, $course2->id);
    ...
    /**
     * Test that enrolment timestart-timeend is respected in metagroup course.
     */
    public function test_timeend() {
        ...
        // Create instance of enrol_metagroup in course2 when there are no enrolments present.
        $metagroup2id = $metagrouplplugin->add_instance($course2, array('customint1' => $course1->id));
        ...
        // Make sure that the same enrolments are now present in course2 under metagroup enrolment.
        $enrolments = $DB->get_records('user_enrolments', array('enrolid' => $metagroup2id), '', 'userid, timestart, timeend, status');
        $this->assertEquals($expectedenrolments, $enrolments);

        // Create instance of enrol_metagroup in course3 and run sync.
        $metagroup3id = $metagrouplplugin->add_instance($course3, array('customint1' => $course1->id));
        enrol_metagroup_sync($course3->id);

        // Make sure that the same enrolments are now present in course3 under metagroup enrolment.
        $enrolments = $DB->get_records('user_enrolments', array('enrolid' => $metagroup3id), '', 'userid, timestart, timeend, status');
        $this->assertEquals($expectedenrolments, $enrolments);
        ...
        // Make sure metagroup courses are also updated.
        $enrolments = $DB->get_records('user_enrolments', array('enrolid' => $metagroup2id), '', 'userid, timestart, timeend, status');
        $this->assertEquals($expectedenrolments, $enrolments);
        $enrolments = $DB->get_records('user_enrolments', array('enrolid' => $metagroup3id), '', 'userid, timestart, timeend, status');
        $this->assertEquals($expectedenrolments, $enrolments);

        // Test metagroup sync. Imagine events are not working.
        $sink = $this->redirectEvents();
        ...
        // Make sure metagroup courses are updated only for the course that was synced.
        enrol_metagroup_sync($course3->id);
        ...
        $enrolmentstatuses = $DB->get_records_menu('user_enrolments', array('enrolid' => $metagroup2id), '', 'userid, status');
        ...
        $manplugin->update_status($manual1, ENROL_INSTANCE_DISABLED);
        enrol_metagroup_sync($course3->id);
        $enrolmentstatuses = $DB->get_records_menu('user_enrolments', array('enrolid' => $metagroup3id), '', 'userid, status');
        $this->assertEquals($allsuspendedenrolemnts, $enrolmentstatuses);
    ...
    public function test_get_user_enrolment_actions() {
        ...
        $pluginname = 'metagroup';

        // Only enable the metagroup enrol plugin.
        ...
        // Teachers don't have enrol/metagroup:unenrol capability by default. Login as admin for simplicity.
        $this->setAdminUser();
        ...
        // metagroup-link enrolment has no enrol actions for active students.
        ...
        $actions = $plugin->get_user_enrolment_actions($manager, $ue);
        // metagroup-link enrolment has enrol actions for suspended students -- unenrol.
